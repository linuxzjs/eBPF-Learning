...
`BCC`是一个框架，它使用户能够编写嵌入了`eBPF`程序的`Python`程序。该框架主要面向涉及应用程序和系统分析/跟踪的用例，其中`eBPF`程序用于收集统计信息或生成事件，而用户空间中的对应部分则负责收集数据并以易于理解的形式显示出来。运行`Python`程序会生成`eBPF`字节码并将其加载到内核中。
...
在`BCC`架构中自定义`BCC`程序：**量化进程内存直接回收延迟问题**;
